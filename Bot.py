...
STOCK_API_URL = "https://script.google.com/macros/s/AKfycbwwQkE9C1U82T0LoYv9umYrc-pmeD0KSZP0IOWAtEvWrKGagUNPJeoUvtIyviQF4-vfoTg/exec"
...
def send_payment_message(chat_id):
    order = orders_db.get(chat_id)
    if not order: return
    items = order.get("items", ""); payable_total = order.get("payable_total", order.get("product_total", order.get("total", 0))); delivery_title = order.get("delivery_title"); delivery_price = order.get("delivery_price"); delivery_service = order.get("delivery_service"); delivery_address = order.get("delivery_address"); total = order.get("total", payable_total)
...
def handle_delivery(message):
    ...
    payable_total = order.get("payable_total", order.get("product_total", order.get("total", 0))); order["total"] = payable_total if selected["price"] is None else payable_total + selected["price"]
...
def handle_contact(message):
    ...
    if order.get("paid_status") or order.get("mixed_order"): send_final_order_message(chat_id)
